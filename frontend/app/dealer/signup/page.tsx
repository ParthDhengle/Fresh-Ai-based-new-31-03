...
export default function DealerSignupPage() {
  ...
  async function onSubmit(values: DealerFormValues) {
    ...
    try {
      ...
      // const response = await fetch('http://localhost:8000/dealer/signup', {
      //   method: 'POST',
      //   headers: {
      ...
      const mockData = {
        ...
        dealer_id: Math.floor(Math.random() * 1000).toString(),
      }

      // Store dealer ID in localStorage
      localStorage.setItem("dealerId", mockData.dealer_id)
      localStorage.setItem("dealerName", values.name)
  ...
  return (
    <div className="container mx-auto py-12 px-4">
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          <CardTitle className="text-2xl">Dealer Signup</CardTitle>
          <CardDescription>
            Create a dealer account to manage your inventory and connect with shopkeepers
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
